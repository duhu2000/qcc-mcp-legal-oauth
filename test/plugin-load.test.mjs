...
if (!skipIfNoDeps()) {
  ...
  test('存储域 spec 合法且 grant schema 可解析', () => {
    ...
    assert.equal(spec.name, 'qcc_legal_mcp_oauth');
    ...
    const grant = buildGrant({
      ...
      authorizedResources: ['https://agent.qcc.com/mcp/regulation/stream'],
      entryResource: 'https://agent.qcc.com/mcp/regulation/stream',
      clientName: 'test',
    });
