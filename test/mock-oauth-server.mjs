...
const RESOURCES = ['legal-regulation', 'legal-case'];
 ...
 * @param {string[]} [options.tokenResources] 若提供，签发的 access_token 为带 resource claim 的 JWT（值传 server key 数组，如 ['legal-regulation','legal-case']，模拟授权范围）
 */
  ...
  const makeAccessToken = () => {
    if (tokenResources) {
      ...
      const payload = Buffer.from(
        JSON.stringify({ client_name: 'DeepSeek Harness - QCC Legal MCP', scope: 'mcp:tools', resource: tokenResources.map((r) => `${base}/mcp/${r}/stream`) }),
      ).toString('base64url');
