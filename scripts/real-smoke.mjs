 ...
 *   3. 用真实 access_token 调真实 MCP：initialize + tools/list（legal-regulation），打印工具数量
...
const ENTRY = QCC_RESOURCES['legal-regulation'];
...
console.log(`[smoke] legal-regulation 服务工具数量: ${toolList.length}`);
...
const result = {
  ...
  regulationToolCount: toolList.length,
