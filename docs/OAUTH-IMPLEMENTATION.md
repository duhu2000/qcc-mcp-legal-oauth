...
对照《企查查MCP OAuth 接入文档》逐条说明本插件的实现与落点（法律数据 MCP，2 个 SERVER）。

## 依赖实现

| 文档章节 | 能力 | 实现位置 | 说明 |
| --- | --- | --- | --- |
| §2 对外服务地址 | 授权集合（2 个 resource） | `lib/constants.js` `QCC_RESOURCES` | legal-regulation（法规）/ legal-case（案例） |
...
- 条目 id：`mcp-qcc-legal-regulation` / `mcp-qcc-legal-case`（`mcpEntryPrefix='mcp-qcc'` + serverKey）；`serverName` = `legal-regulation` / `legal-case` → 工具前缀 `mcp__legal-regulation__*` / `mcp__legal-case__*`。

### 3. Token 持久化：`ctx.storageDomain`
- 域 `qcc_legal_mcp_oauth`，表 `grants`，记录 schema 见 `lib/grant-store.js`；
- 落盘位置 `~/.dsh/storages`（目录 0700）；
- 与企业数据 MCP 插件（域 `qcc_mcp_oauth`）完全隔离，可同时安装互不干扰。
...
- mock 集成：动态注册、完整授权码流程（loopback）、PKCE 不匹配、code 单次使用、refresh 轮换（旧 token 失效）、revoke 后刷新拒绝、MCP 401 challenge 响应头、`extractTokenResources`（法规+案例 / 仅法规）
- 插件级：一键连接（自动模拟用户授权）、授权持久化 + 2 条目创建、重复连接幂等、过期自动刷新（token 轮换 + 条目更新）、断开（revoke + 清除 + 停用）、重启恢复、按 token 授权动态挂载（仅法规账号不挂 case）
- 冒烟：插件模块在 DSH 真实 Host 中加载并执行 `apply()`（隔离 dev profile 实测）

## 未覆盖 / 需确认

- 真实 `agent.qcc.com` 端到端授权（需要真实账号在浏览器点击授权）—— 见 `docs/INSTALL.md` 使用流程 / `scripts/real-smoke.mjs`
- 账号开通范围是否「法规+案例」同时授权，以 token `resource` claim 实际值为准（插件已按 claim 动态挂载，天然兼容）
