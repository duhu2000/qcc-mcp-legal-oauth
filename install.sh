#!/usr/bin/env bash
# qcc-dsh-mcp-legal-oauth 一键安装脚本（DeepSeek Harness）
# 用法：
#   本地：  bash install.sh
#   远端：  bash <(curl -fsSL https://raw.githubusercontent.com/duhu2000/qcc-mcp-legal-oauth/main/install.sh)
# 支持：优先 dsh CLI（自动注册 bundle）；无 dsh 时回退 pnpm 并兜底注册 bundles。
# 幂等：重复执行不会重复注册或破坏已有配置。
set -euo pipefail

PKG="qcc-dsh-mcp-legal-oauth"
...
echo "   重启后在对话中输入「连接企查查法律数据」即可一键 OAuth 授权使用企查查法律数据 MCP 服务。"
echo "   详见：https://github.com/duhu2000/qcc-mcp-legal-oauth"
