 ...
 * （企查查法律数据 MCP：独立存储域，与公司 MCP 插件 qcc_mcp_oauth 互不干扰）
 */
import { z } from 'zod';
...
/** 定义 qcc-legal-mcp-oauth 存储域（单表 grants） */
export function defineQccGrantDomain() {
  return defineDomain({
    name: 'qcc_legal_mcp_oauth',
    version: 1,
    tables: {
...
export class GrantStore {
  constructor(domain) {
    ...
    if (!this.table) throw new Error(`qcc-legal-mcp-oauth: domain table 'grants' not found`);
  }
