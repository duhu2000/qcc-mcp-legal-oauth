 ...
 * 例：https://agent.qcc.com/mcp/regulation/stream
...
/** base64url 解码为 UTF-8 字符串（补齐 padding 后按标准 base64 解）。 */
...
/**
 * 从 JWT access_token 提取实际授权的 resource URL 列表（RFC 9068：resource 授权范围在 payload.resource）。
 * 企查查 access_token 为 JWT：账号 token 的 resource 可能同时含 legal-regulation + legal-case，也可能只含其一。
 * 解析失败（非 JWT / 无 resource claim）返回 null，由调用方 fallback 到全部资源。
 * @param {string} accessToken
 * @returns {string[]|null} 实际授权的 resource URL 列表；无法解析时返回 null
 */
export function extractTokenResources(accessToken) {
  try {
    ...
    // RFC 9068：resource 授权范围既可以是字符串数组，也可以是单个字符串
