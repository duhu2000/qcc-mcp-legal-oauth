/**
 * 企查查法律数据 MCP OAuth 插件（DeepSeek Harness / Cordis 插件）
 *
 * 能力：
 *   - 一键 OAuth 授权（Authorization Code + PKCE S256，动态注册客户端，无 client_secret）
 *   - 一次授权覆盖企查查法律数据 MCP SERVER（legal-regulation 法规 / legal-case 案例，共 2 个；按 token 实际授权动态配置）
 ...
 *
 * 依据《企查查MCP OAuth 接入文档》（法律数据 MCP：https://agent.qcc.com/legal）
 */
...
import { provisionEntries, disableEntries, entryIdFor, grantedServerKeys } from './mcp-provision.js';
...
export const name = 'qcc-legal-mcp-oauth';
...
export const Config = z.object({
  ...
  clientName: z.string().default('DeepSeek Harness - QCC Legal MCP'),
  /** loopback 回调路径 */
  ...
  /** 激活时若无有效授权是否自动发起 OAuth 授权（自动打开授权页，可配置关闭） */
  autoConnectOnActivate: z.boolean().default(true),
...
export async function apply(ctx, config) {
  const logger = ctx.logger('qcc-legal-mcp-oauth');

  const domain = await ctx.storageDomain.open(defineQccGrantDomain());
  // 注意：ctx.effect 会「立即执行」回调，回调的返回值才是 fiber 卸载时的清理函数。
  // 写成 `ctx.effect(() => { domain.close() })` 会在 apply 期间立即关闭存储域，
  // 导致后续 store.put/get 全部抛 DomainError('closed')（授权不落盘、重启不恢复）。
  ctx.effect(() => () => {
  ...
  async function fullConnect(server, signal) {
    ...
    // 按 token 实际授权范围过滤要配置的 server：账号 token 的 resource 可能同时含
    // legal-regulation + legal-case（2 个），也可能只含其一——只配置实际授权的条目，
    // 避免把未授权的 resource 挂出来却调不通。
    // 解析失败（非 JWT）时 fallback 到全部 config.resources。
    const grantedUrls = extractTokenResources(token.accessToken);
    const resourceEntries = Object.entries(config.resources);
    let serverKeys = resourceEntries.map(([key]) => key);
    if (grantedUrls) {
      const granted = new Set(grantedUrls);
      serverKeys = resourceEntries.filter(([, url]) => granted.has(url)).map(([key]) => key);
      // 兜底：授权响应异常导致空集时，至少保留入口 server
      if (serverKeys.length === 0 && config.resources[server]) serverKeys = [server];
    }

    // 一次授权覆盖 token 实际授权的 resource 集合（文档 §11）
    const grant = buildGrant({
      issuer,
      clientId: registration.clientId,
      scope: token.scope ?? DEFAULT_SCOPE,
      token,
      authorizedResources: serverKeys.map((key) => config.resources[key]),
  ...
  const api = {
    async connect(server = 'legal-regulation', signal) {
      try {
        // 已有授权：优先复用/刷新（文档 §11.2），避免重复弹授权页
        if (state.grant) {
          if (state.grant.accessTokenExpiresAt > Date.now()) {
            await provisionEntries(ctx, config, state.grant);
            const keys = grantedServerKeys(config, state.grant);
            return { ok: true, message: `企查查法律数据 MCP 已连接（复用现有授权，覆盖 ${keys.length} 个 Server：${keys.join('/')}）`, detail: { reused: true, servers: keys } };
          }
          try {
            await refreshGrant();
            return { ok: true, message: '企查查法律数据 MCP 已连接（access_token 已自动刷新）', detail: { refreshed: true } };
          } catch (error) {
            logger.warn(`refresh failed, re-authorizing: ${error.message}`);
          }
        }
        const { grant, ids } = await fullConnect(server, signal);
        const keys = grantedServerKeys(config, grant);
        return {
          ok: true,
          message: `企查查法律数据 MCP 授权成功，已连接 ${ids.length} 个 MCP Server（${keys.join('/')}），可以直接使用 mcp__legal-* 工具`,
          detail: { clientId: grant.clientId, expiresAt: grant.accessTokenExpiresAt, entries: ids, servers: keys },
        };
      } catch (error) {
        logger.error(`connect failed: ${error.message}`);
        return {
          ok: false,
          message: `企查查法律数据 MCP 连接失败：${error.message}${error instanceof OAuthError && error.code === 'invalid_grant' ? '（授权已失效，请重试连接）' : ''}`,
    ...
    async status() {
      ...
      if (!grant) {
        return { ok: true, message: '企查查法律数据 MCP 未连接。对我说"连接企查查法律数据"即可一键 OAuth 授权。', detail: { connected: false } };
      }
      const now = Date.now();
      ...
      return {
        ok: true,
        message: [
          `企查查法律数据 MCP 已连接：${grant.clientName}`,
          `授权覆盖 ${keys.length} 个 MCP Server：${keys.join('/')}`,
          `access_token 过期时间：${new Date(grant.accessTokenExpiresAt).toLocaleString()}${expired ? '（已过期）' : ''}`,
          state.needsReauth ? '⚠️ 需要重新授权（对我说"连接企查查法律数据"）' : '状态正常，token 自动刷新中',
        ].join('；'),
        detail: {
          connected: true,
          clientId: grant.clientId,
    ...
    async disconnect() {
      ...
      return {
        ok: true,
        message: `企查查法律数据 MCP 已断开：refresh_token ${revoked ? '已撤销' : '已清除（撤销失败，本机凭证已删除）'}，${ids.length} 个 MCP Server 已停用`,
  ...
  // 同样：ctx.effect 立即执行回调，必须「返回」卸载函数而不是直接调用它——
  // 写成 `ctx.effect(() => disposerTools())` 会在注册瞬间卸载全部工具（agent 看不到 qcc 工具）。
  ...
  if (config.persistTokens) {
    ...
    if (stored) {
      ...
      try {
        ...
        logger.info(`restored QCC legal MCP authorization for account '${config.account}'`);
      } catch (error) {
        state.needsReauth = true;
        logger.warn(`restore failed (${error.message}); run qcc_legal_oauth_connect to re-authorize`);
      }
    }
  }

  // fiber 卸载时清理刷新定时器（同样需「返回」清理函数，而不是在 apply 期间执行）
  ctx.effect(() => () => {
    clearTimeout(state.refreshTimer);
  });

  // 激活且无有效授权时自动发起 OAuth 授权（默认开启；可用 autoConnectOnActivate: false 关闭）。
  // 不阻塞 apply：工具照常注册，授权完成后再动态配置 mcp-client 条目。
  if (!state.grant && config.autoConnectOnActivate) {
    logger.info('no valid QCC legal authorization found — auto-starting OAuth connect (disable with autoConnectOnActivate: false)');
    const signal = new AbortController().signal;
    api.connect('legal-regulation', signal).then((result) => {
      if (result.ok) logger.info(`auto-connect succeeded: ${result.message}`);
      else logger.warn(`auto-connect did not complete: ${result.message}`);
    }).catch((error) => {
      logger.warn(`auto-connect errored: ${error.message}`);
    });
  }

  logger.info(`qcc-legal-mcp-oauth plugin active (manages ${Object.keys(config.resources).length} QCC legal MCP servers)`);
