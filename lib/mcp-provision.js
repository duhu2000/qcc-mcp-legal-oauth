...
function buildEntryConfig(config, serverKey, grant) {
  ...
  if (!url) throw new Error(`qcc-legal-mcp-oauth: no resource url for server '${serverKey}'`);
  return {
 ...
 * 法律数据 MCP：账号 token 的 resource 可能同时含 legal-regulation + legal-case（2 个），
 * 也可能只含其一；grant 为 null 时 fallback 到全部 config.resources。
 ...
 * serverKeys 由 grant.authorizedResources 反推（账号 token 可能含 legal-regulation + legal-case = 2 个，
 * 也可能只含其一），只配置已授权的条目，避免把未授权的 resource 挂出来却调不通。
